...
public class FastDynamicBoneManager : MonoBehaviour
{
    ...
    FastDynamicBoneCacheLevel GetCacheLevel(DynamicBone.ParticleTree tree)
    {
        ...
        else
        {
            if (Application.isEditor && pCount > cacheLevelMaxParmas.w)
            {
                Debug.LogWarningFormat(
                    "DynamicBone : some one root's Particles Count = {0} greater than MaxParticleLimit = {1}",
    ...
    public void ReadBone(DynamicBone target)
    {
        if (treeCount <= 0) return;
        int index = dynamicBoneList.IndexOf(target.GetHashCode());
        if (index == -1) return;
    ...
    public void UpdateBone(DynamicBone target)
    {
        if (treeCount <= 0) return;
        int index = dynamicBoneList.IndexOf(target.GetHashCode());
        if (index == -1) return;
    ...
    private bool RemoveTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
    ...
    private bool AddTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
        FastDynamicBoneCacheLevel level = GetCacheLevel(tree);
    ...
    private void ReadTree(DynamicBone.ParticleTree tree)
    {
        FastDynamicBoneCacheLevel level = GetCacheLevel(tree);
    ...
    private void UpdateTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
        FastDynamicBoneCacheLevel level = GetCacheLevel(tree);
