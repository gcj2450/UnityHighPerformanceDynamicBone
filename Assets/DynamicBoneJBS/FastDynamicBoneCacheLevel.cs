...
public class FastDynamicBoneCacheLevel
{
    ...
    public bool RemoveTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
    ...
    public bool AddTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
        ...
        for (int i = 0; i < MaxColliderLimit; i++)
        {
            if (i < cCount)
            {
                ...
                {
                    Debug.LogErrorFormat("DynamicBone : Collider has someone null.");
                }
            }
            else
            {
    ...
    public void ReadTree(DynamicBone.ParticleTree tree)
    {
        int index = treeList.IndexOf(tree.GetHashCode());
        if (index == -1) return;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            DynamicBone.Particle p = tree.m_Particles[i];
    ...
    public void UpdateTree(DynamicBone.ParticleTree tree, DynamicBone target)
    {
        int index = treeList.IndexOf(tree.GetHashCode());
        if (index == -1) return;
                    ...
                    for (int i = 0; i < pMaxParticleLimit; i++)
                    {
                        ...
                        if (i < head.m_ParticleCount)
                        {
                            ...
                            if (i > 0)
                            {
                                ...
                                DynamicBone.FreezeAxis freezeAxis = head.m_FreezeAxis;
                                if (freezeAxis != DynamicBone.FreezeAxis.None)
                                {
                                    float3 planeNormal;
